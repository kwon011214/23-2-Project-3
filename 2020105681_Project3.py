    ...
    for i in range(len(balls)):
        if collision(cueBall, balls[i]):
            if balls[i].x == cueBall.x:
                ...
                balls[i].speed = ((v1*cos(radians(balls[i].angle)))**2 + (v2*sin(radians(cueBall.angle)))**2)**0.5
                cueBall.speed = ((v2*cos(radians(cueBall.angle)))**2 + (v1*sin(radians(balls[i].angle)))**2)**0.5
        ...
        for j in range(len(balls) - 1, i, -1):
            if collision(balls[i], balls[j]):
                if balls[i].x == balls[j].x:
                    ...
                    balls[i].speed = ((v1*cos(radians(balls[i].angle)))**2 + (v2*sin(radians(balls[j].angle)))**2)**0.5
                    balls[j].speed = ((v2*cos(radians(balls[j].angle)))**2 + (v1*sin(radians(balls[i].angle)))**2)**0.5
...
def gameOver():
    ...
    if len(balls) == 0:
        text = font.render("You Win!", True, (133, 193, 233))
    else:
        text = font.render("You Lose! Black Ball in Hole!", True, (241, 148, 138))
...
def poolTable():
    ...
    p1 = Pocket(0, 0, black)
    p2 = Pocket(width/2 - p1.r*2, 0, black)
    p3 = Pocket(width - p1.r - margin - 5, 0, black)
    p4 = Pocket(0, height - margin - 5 - p1.r, black)
    p5 = Pocket(width/2 - p1.r*2, height - margin - 5 - p1.r, black)
    p6 = Pocket(width - p1.r - margin - 5, height - margin - 5 - p1.r, black)
